...
def extract_metadata(header_file):
    ...
    excluded_fields = set([
        ...
        'tocodynamometer was repositioned at 00'
    ])
...
def main(data_dir, output_npy1, output_npy2):
    """Process all records in the directory and save to two .npy files."""
    ...
    mid_idx = len(data) // 2
    np.save(output_npy1, np.array(data[:mid_idx], dtype=object))
    np.save(output_npy2, np.array(data[mid_idx:], dtype=object))
    
    print(f"Successfully saved first half to {output_npy1}")
    print(f"Successfully saved second half to {output_npy2}")

if __name__ == "__main__":
    data_dir = os.path.join("..", "Master-Thesis-Data", "ehgdb")
    output_npy1 = os.path.join(data_dir, "ehgdb_database_part1.npy")
    output_npy2 = os.path.join(data_dir, "ehgdb_database_part2.npy")
    main(data_dir, output_npy1, output_npy2)
